          ...
          $.ajax({
            ...
            success: function (response) {
              if (response == 1) {
                ...
                swal(
                  "Can not be deleted",
                  "Station is currently in use in Original PAR/ICS."
                );
              } else if (response == 3) {
                swal(
                  "Can not be deleted",
                  "Station is currently in use in Old PAR/ICS."
                );
              } else if (response == 4) {
                swal(
                  "Can not be deleted",
                  "Station is currently in use in monitoring station deployed equipment "
                );
              } else if (response == "Invalid input") {
      ...
      success: function (response) {
        if (response.status === "success") {
          ...
          $.each(response.result, function (index, item) {
            options +=
              '<option value="' + item.code_id + '">' + item.code + "</option>";
          });
          ...
          $.ajax({
            ...
            success: function (response) {
              if (response == 1) {
                ...
                swal("Can not be deleted", "Station Code is currently in used");
              } else if (response == 3) {
                swal(
                  "Can not be deleted",
                  "Station Code is  currently in used"
                );
              } else if (response == 4) {
  ...
  $("#btnUpdateCode").click(function (event) {
    ...
    if (checkAndUpdateCode()) {
      ...
      bootbox.confirm("Do you really want to update?", function (result) {
        if (result) {
          $.ajax({
            url: "update_Station_Code.php",
            type: "POST",
            data: {
              upd_code_id: codeSelectedID,
...
function hideshow(elementId) {
  var div = document.getElementById("tranStationSection");
  var div1 = document.getElementById("CodeSection");
  var element = document.getElementById(elementId);

  if (elementId === "tranStationSection") {
    div.style.display = "block";
    div1.style.display = "none";
  } else if (elementId === "CodeSection") {
