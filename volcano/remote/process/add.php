...
try {
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $insert = "INSERT INTO tbl_remote_station (`station`, `station_type`, `station_category`, `station_name`, `latitude`, `longitude`, `elevation`, `instrument`, `date_started`, `date_destroyed`, `remarks`, `record_status`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($insert);
        $stmt->bind_param("iiisssssssss", $station, $stationType, $stationCategory, $stationName, $latitude, $longitude, $elevation, $instrument, $dateStarted, $dateDestroyed, $remarks, $recStatus);

        if ($stmt->execute()) {
            $insertedId = $conn->insert_id; #Get the last inserted ID
            $insert = "INSERT INTO tbl_remote_address (`remote_station_id`, `address1`, `region`, `province`, `municipality`, `barangay`) VALUES (?, ?, ?, ?, ?,?)";
            $stmt = $conn->prepare($insert);
            $stmt->bind_param("isssss", $insertedId, $address, $region, $province, $municipality, $barangay);
            if ($stmt->execute()) {
                $stmt->close();
