...
try {
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $id = $_SESSION['update_transaction_id'];
        ...
        if (isset($_FILES['uploadFile']) && $_FILES['uploadFile']['size'] > 0) {
            ...
            $update = "UPDATE tbl_oldtransaction 
               SET `Item_Name` = ?, `Serial_Number` = ?, `Property_Number` = ?, `qty` = ?, `Transferred_From` = ?, `Transferred_To` = ?, `Division_id` = ?, `Station_Name` = ?, `station_code_id` = ?, `Land_Owner` = ?, `Care_Taker` = ?, `Installation_Date` = ?, `Date_Acquired` = ?, `Status` = ?, `Remarks` = ?, `Filename` = ?, `Filepath` = ?
               WHERE `id` = ?";
            $stmt = mysqli_prepare($conn, $update);
            $stmt->bind_param("sssissiiissssssssi", $ItemName, $SerialNum, $PropertyNum, $qty, $transFrom, $transTo, $divisionId, $stationId, $stationCodeId, $landOwner, $careTaker, $installDateConverted, $dateAcquired, $status, $rem, $rawFileName, $targetPath, $id);
        } else {
            #no file query
            $update = "UPDATE tbl_oldtransaction 
                   SET `Item_Name` = ?, `Serial_Number` = ?, `Property_Number` = ?, `qty` = ?, `Transferred_From` = ?, `Transferred_To` = ?, `Division_id` = ?, `Station_Name` = ?, `station_code_id` = ?, `Land_Owner` = ?, `Care_Taker` = ?, `Installation_Date` = ?, `Date_Acquired` = ?, `Status` = ?, `Remarks` = ?
                   WHERE `id` = ?";
            $stmt = mysqli_prepare($conn, $update);
            $stmt->bind_param("sssissiiissssssi", $ItemName, $SerialNum, $PropertyNum, $qty, $transFrom, $transTo, $divisionId, $stationId, $stationCodeId, $landOwner, $careTaker, $installDateConverted, $dateAcquired, $status, $rem, $id);
        }
