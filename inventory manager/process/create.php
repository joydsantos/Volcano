...
try {
    ...
    $endorsed_by = intval($_POST['endorsed_by']);
    $par = intval($_POST['par']);
    $station_id = intval($_POST['station_id']);

    $insertICTEquipment = "INSERT INTO `tbl_inventory` (`type_id`, `item_name`, `brand_model`, `serial_no`, `quantity`, `unit_id`, `acquisition_year`, `deployment`, `condition_id`, `action_taken`, `remarks`, `endorsed_by`, `par`,`station_id`)  
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";


    $stmt = $conn->prepare($insertICTEquipment);
    $stmt->bind_param("isssiisssssiii", $type_id, $name, $brand_model, $serial_no, $quantity, $unit,  $acquisition_year, $deployment, $condition_id, $action_taken, $remarks, $endorsed_by, $par, $station_id);
    $result = $stmt->execute();
    ...
    LEFT join tbl_par p on p.id = i.par WHERE i.`id` = $lastInsertID";
